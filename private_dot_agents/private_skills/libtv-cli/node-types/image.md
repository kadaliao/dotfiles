...
| ---------------------- | --------------------------------------------------------------------------------------------------------------------------------------- |
| **`prompt`**           | 提示词。                                                                                                                                |
| **`model`**            | 图片模型 **名字（`modelName`）**，如 `-s "model=LibNano Pro"`（用 `libtv model search --type image` 查看；CLI 内部解析为 `modelKey`）。 |
...
> 生成参数（`prompt` / `model` / `ratio` / `quality` 等）请用 `-s`；展示名用 `--name`，不要用 `-u` 改名，见 [README.md](./README.md) 的「`-u` 明确拒写的键」。
