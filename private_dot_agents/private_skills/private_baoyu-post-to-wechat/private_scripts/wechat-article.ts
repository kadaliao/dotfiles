...
export async function postArticle(options: ArticleOptions): Promise<void> {
  ...
  try {
    ...
    if (effectiveTitle) {
      const finalTitle = await evaluate<string>(session, `document.querySelector('#title')?.value || ''`);
      if (finalTitle !== effectiveTitle) {
        throw new Error(`Title changed before save. Expected "${effectiveTitle}", got ${finalTitle.length} chars.`);
      }
      console.log('[wechat] Final title verified OK before save.');
    }
